...
function inicio(){
	...
	calacas = [new Calaca(), new Calaca(),
				   ...
				   new Calaca(), new Calaca(), new Calaca(), new Calaca(), new Calaca()];
	run();	
...
function run(){ 
	...
	if(jugando){  
		...
		contextoBuffer.clearRect(0,0,buffer.width,buffer.height);
		contextoBuffer.fillStyle = "#4e4eae";
		quica.sprite = 3;
		quica.vida = 0;
		quica.dibujar(contextoBuffer);
		contextoBuffer.font = "50px sans-serif";
		contextoBuffer.fillText("JUEGO TERMINADO", 110, 100);
		contextoBuffer.fillStyle = "#ec7e35";
